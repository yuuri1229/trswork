    ...
    <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-slate-200 sm:p-6">
      ...
      <form onSubmit={handleSave} className="mt-4 flex flex-col gap-4">
        ...
        <div className="rounded-xl bg-slate-50 p-4">
          <p className="text-sm font-medium text-slate-600">Google スプレッドシート連携</p>
          <p className="mt-1 text-xs text-slate-400">
            Google Apps Script（google-apps-script/Code.gs）をウェブアプリとしてデプロイし、発行された URL を貼り付けてください。詳しくは README を参照してください。
          </p>

          <label className="mt-3 flex flex-col gap-1 text-sm">
            <span className="font-medium text-slate-600">Web アプリ URL</span>
            <input
              value={draft.sheetsWebAppUrl}
              onChange={(e) => setDraft({ ...draft, sheetsWebAppUrl: e.target.value })}
              placeholder="https://script.google.com/macros/s/xxxx/exec"
              className="rounded-lg border border-slate-300 px-3 py-2 text-xs focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-100"
            />
          </label>

          <label className="mt-3 flex flex-col gap-1 text-sm">
            <span className="font-medium text-slate-600">共有シークレット（任意）</span>
            <input
          ...
          <label className="mt-3 flex items-center gap-2 text-sm">
            ...
            />
            <span className="text-slate-600">作業終了時に自動でスプレッドシートへ反映する</span>
          </label>
        </div>
      ...
      {unsyncedEntries.length > 0 && (
        <div className="mt-6 border-t border-slate-100 pt-4">
          <p className="text-sm font-medium text-slate-600">未同期の記録 ({unsyncedEntries.length}件)</p>
            ...
            {unsyncedEntries.map((entry) => (
              <li key={entry.id} className="flex items-center justify-between gap-2 text-xs">
                ...
                <button
                  ...
                  className="shrink-0 rounded-md bg-slate-100 px-2 py-1 font-medium text-slate-600 hover:bg-slate-200 disabled:opacity-50"
                >
